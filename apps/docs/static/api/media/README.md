...
A subset of the toolchain (`rolldown`, the `@biomejs/cli-linux-x64`
glibc variant, Vite 6's native bindings) ships binaries linked
against glibc 2.28 or newer. On Linux distributions that still ship
glibc 2.26 (notably Amazon Linux 2), those binaries cannot load
directly. `scripts/docker.sh` runs the monorepo inside a pinned
Debian Bookworm image (glibc 2.36, Node 22.22.2) that matches CI.

```bash
./scripts/docker.sh ci           # full pipeline: lint + check:banned + typecheck + build + test
./scripts/docker.sh build        # npm ci && npm run build
./scripts/docker.sh test         # npm ci && npm run test
./scripts/docker.sh check:spec   # conformance regression guard
./scripts/docker.sh check:perf   # throughput regression gate
./scripts/docker.sh shell        # drop into a ready-to-use shell
./scripts/docker.sh npm run -w @streamd/parser test   # forward any command
```

The image is built once on first use and reused across invocations.
Workspace files are bind-mounted — edits made on the host are visible
inside the container immediately. The npm cache lives at
`.cache/docker-npm/` in the workspace (gitignored) so it inherits
host ownership via the bind mount.

Contributors on glibc-2.28+ distributions and macOS (via Docker
Desktop) can use this wrapper too; it produces the same artifacts
everywhere.
