/**
 * Security tests for the `meta.attrs` allowlist and the deprecated
 * `allowDangerousMetaHtml` migration signal.
 *
 * Covers:
 ...
 * - Passing `allowDangerousMetaHtml` throws `StreamdHtmlArgumentError`.
 *
...
describe("allowDangerousMetaHtml — deprecated option throws", () => {
  it("throws StreamdHtmlArgumentError when allowDangerousMetaHtml is passed to renderHtml", () => {
    expect(() =>
      renderHtml([], { allowDangerousMetaHtml: true } as Record<string, unknown>),
  ...
  it("throws with a message mentioning schema 2 and components override", () => {
    expect(() =>
      renderHtml([], { allowDangerousMetaHtml: false } as Record<string, unknown>),
    ).toThrow(/schema 2/);
  });
});
